...
// Lightweight stand-ins for baguette's logging helpers — we intentionally swallow chatter
// in a library context. Wire to os_log if you need visibility.
@inline(__always) private func _log(_ s: String) {}
@inline(__always) private func _logErr(_ s: String) {}
...
final class HIDInput: @unchecked Sendable {
    ...
    func key(_ key: KeyboardKey, modifiers: Set<KeyModifier>, duration: Double) -> Bool {
        guard let c = ensureWarm(), let kfn = hidArbFn else {
            _log("[hid] key — IndigoHIDMessageForHIDArbitrary unresolved")
            return false
        }
        let holdUs = holdMicroseconds(for: duration)
        ...
        _log("[hid] key page=\(key.hidUsage.page) usage=\(key.hidUsage.usage) modifiers=\(mods.map(\.rawValue)) hold=\(holdUs)us")
    ...
    private func pressLegacyButton(_ button: DeviceButton, holdUs: UInt32, on client: AnyObject) -> Bool {
        guard let bfn = buttonFn else {
            _log("[hid] press \(button.rawValue) — buttonFn unresolved")
            return false
        }
        let (arg0, target) = buttonCodes(for: button)
        _log("[hid] press \(button.rawValue) via legacy arg0=\(arg0) target=0x\(String(target, radix: 16)) hold=\(holdUs)us")
        guard let down = bfn(arg0, 1, target) else {
            _log("[hid] press \(button.rawValue) — down message build returned nil")
            return false
        }
        ...
        guard let up = bfn(arg0, 2, target) else {
            _log("[hid] press \(button.rawValue) — up message build returned nil")
            return false
        }
    ...
    private func swipeFromBottomEdge(on client: AnyObject, hold: Bool) -> Bool {
        guard mouseEdgeFn != nil else {
            _log("[hid] swipe-from-bottom-edge — mouseEdgeFn unresolved")
            return false
        }
        let xN = 0.5
        let yStart = 0.95
    ...
    private func pressArbitraryHID(_ button: DeviceButton, usage: HIDUsage, holdUs: UInt32, on client: AnyObject) -> Bool {
        guard let kfn = hidArbFn else {
            _log("[hid] press \(button.rawValue) — IndigoHIDMessageForHIDArbitrary unresolved")
            return false
        }
        let target = Self.touchDigitizer
        _log("[hid] press \(button.rawValue) target=0x\(String(target, radix: 16)) page=\(usage.page) usage=\(usage.usage) hold=\(holdUs)us")
        ...
        send(message: up, to: client)
        _log("[hid] press \(button.rawValue) — sent down+up")
        return true
    }
    ...
    private func ensureWarm() -> AnyObject? {
        ...
        guard let cls = NSClassFromString("_TtC12SimulatorKit24SimDeviceLegacyHIDClient") else {
            _logErr("SimDeviceLegacyHIDClient class not found")
            return nil
        }
        ...
        guard let c = initFn(allocated, initSel, device, &err) else {
            if let err { _logErr("SimDeviceLegacyHIDClient init failed: \(err)") }
            return nil
        }
        client = c
        warmServices(on: c)
    ...
    private func resolveFunctions() {
        ...
        guard let handle = dlopen(path, RTLD_NOW) else {
            _logErr("SimulatorKit dlopen failed: \(dlerrorString())")
            return
        }
        ...
        _log("[hid] symbols resolved — mouse:\(mouseFn != nil) mouseEdge:\(mouseEdgeFn != nil) button:\(buttonFn != nil) hidArb:\(hidArbFn != nil) scroll:\(scrollFn != nil)")
    }
